...
class MCBot:
    ...
    async def assign(self,ctx):
        ...
        await self.send_message(self, ctx, "Assigned to " + str(ctx.message.channel.name))
    ...
    async def pingtrello(self, ctx):
        ...
        if(check_trello()):
            ...
            await self.send_embed(self, ctx, embed)
        else:
    ...
    async def token(self, ctx):
        ...
        await send_message(self, ctx, "https://trello.com/1/authorize?expiration=never&scope=read,write,account&response_type=token&name=Moon%20CommandBot&key="+self.trello_ky)
    @commands.command()
    async def link(self, ctx, api_token):
        ...
        await send_message(self, ctx, "Received token of Token: " + self.trello_tk)
    def check_trello():
